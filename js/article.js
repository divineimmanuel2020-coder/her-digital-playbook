...
const DIFFICULTY_LEVEL = { 'Beginner Friendly': 3, Intermediate: 7, Advanced: 11 };

function heroLevelCardHtml(item) {
  const level = DIFFICULTY_LEVEL[item.difficulty] || 3;
  return `
    <div class="hero-level-card">
      <span class="level-pill">⭐ LEVEL ${level}</span>
      <h1 class="hero-level-title">${item.title}</h1>
      <p class="hero-level-desc">${item.excerpt}</p>
      <div class="hero-level-chips">
        <span class="hero-chip">🕐 ${item.readTime || 'Self-paced'}<br><small>Estimated Time</small></span>
        <span class="hero-chip">⭐ 25 XP<br><small>Reward</small></span>
        <span class="hero-chip">📶 ${item.difficulty || 'Beginner Friendly'}<br><small>Level</small></span>
